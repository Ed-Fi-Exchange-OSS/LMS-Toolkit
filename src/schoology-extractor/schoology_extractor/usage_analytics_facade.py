...
def get_system_activities(usage_input_dir: str, db_engine: sqlalchemy.engine.base.Engine) -> pd.DataFrame:
    """
    ...
    output = pd.DataFrame()
    logger.debug(f"Processing usage analytics files: loading files from {usage_input_dir}")
    for file in os.scandir(usage_input_dir):
        ...
        if not sync.usage_file_is_processed(file.name, db_engine):
            ...
            logger.debug(f"Ignoring usage analytics file because it has already been processed: {file.name}")
