...
def test_overlap_removal(mock_latest_teachers_df, test_db_fixture):
    # 1st pull: 17 rows
    mock_latest_teachers_df.return_value = pd.read_csv(
        "tests/api/teachers/teachers-1st.csv"
    )
    first_teachers_df = request_all_teachers_as_df(None, DUMMY_COURSE_IDS, test_db_fixture)
    ...
    second_teachers_df = request_all_teachers_as_df(None, DUMMY_COURSE_IDS, test_db_fixture)
    ...
    third_teachers_df = request_all_teachers_as_df(None, DUMMY_COURSE_IDS, test_db_fixture)
...
@patch("google_classroom_extractor.api.teachers.request_latest_teachers_as_df")
def test_value_replacement(mock_latest_teachers_df, test_db_fixture):
    course_id = "5583344"
    user_id = "79924907"
    ...
    # overwrite pull
    overwrite_teachers_df = request_all_teachers_as_df(None, DUMMY_COURSE_IDS, test_db_fixture)
