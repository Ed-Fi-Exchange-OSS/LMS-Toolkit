...
from edfi_google_classroom_extractor.api.students import request_latest_students_as_df
...
def describe_when_requesting_latest_students():
    ...
    def students_df() -> DataFrame:
        ...
        resource = setup_fake_classroom_api(
            [(f"courses/{COURSE_ID}/students", response_json)]
        )

        # act
        return request_latest_students_as_df(resource, [COURSE_ID])

    def it_should_have_correct_dataframe_shape(students_df):
        row_count, column_count = students_df.shape
        assert row_count == 1
        assert column_count == 7

    def it_should_map_dataframe_columns_correctly(students_df):
        row_dict = students_df.to_dict(orient="records")[0]
