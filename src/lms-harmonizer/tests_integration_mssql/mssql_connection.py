...
class MSSqlConnection(object):
    ...
    def pyodbc_conn(self) -> Iterator[pyodbc.Connection]:
        ...
        if self.config.useintegratedsecurity == "true":
            connection = pyodbc.connect(
                f"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={self.config.server},{self.config.port};DATABASE={self.config.db_name};Trusted_Connection=Yes;"
            )
        else:
            connection = pyodbc.connect(
