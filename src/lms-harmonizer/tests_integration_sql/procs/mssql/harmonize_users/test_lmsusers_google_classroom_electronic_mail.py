...
def describe_when_lms_and_ods_tables_have_no_matches():
    def it_should_run_successfully(test_db_config: ServerConfig):
        ...
        with MSSqlConnection(test_db_config).pyodbc_conn() as connection:
            ...
            insert_edfi_student_electronic_mail(connection, 1, "not_sis_id_1")
            insert_edfi_student_electronic_mail(connection, 2, "not_sis_id_2")
...
def describe_when_lms_and_ods_tables_have_a_match():
    ...
    def it_should_run_successfully(test_db_config: ServerConfig):
        ...
        with MSSqlConnection(test_db_config).pyodbc_conn() as connection:
            ...
            insert_edfi_student_electronic_mail(connection, 1, SIS_ID)
...
def describe_when_single_student_has_multiple_emails_with_one_match():
    ...
    def it_should_run_successfully(test_db_config: ServerConfig):
        ...
        with MSSqlConnection(test_db_config).pyodbc_conn() as connection:
            insert_lms_user(connection, SIS_ID, SOURCE_SYSTEM)
            insert_edfi_student_with_usi(connection, 1, STUDENT_ID)
            insert_edfi_student_electronic_mail(connection, 1, "not_sis_id")
            insert_edfi_student_electronic_mail(connection, 1, SIS_ID)
            insert_edfi_student_electronic_mail(connection, 1, "also_not_sis_id")
