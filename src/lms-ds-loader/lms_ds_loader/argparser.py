...
def parse_arguments(args_in) -> Arguments:
    ...
    parser.add_argument("-c", "--csvpath", help="base path for input files", required=True)
    parser.add_argument("-e", "--engine", help=f"database engine", choices=[ Constants.MSSQL, Constants.POSTGRESQL], default=Constants.MSSQL)
    parser.add_argument("-s", "--server", help="database server name or IP address", required=True)
    ...
    parser.add_argument("-i", USE_INTEGRATED, help="use Integrated Security", action="store_true")
